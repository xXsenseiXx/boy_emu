...
extern SPI_HandleTypeDef hspi2; // Tells this file that hspi1 is defined in another file
...
static int rcvr_datablock_dma(BYTE *buff, UINT btr);
static int sd_read_block_dma(DWORD sector, BYTE *buff);

#define SD_DMA_CHUNK 64u
...
extern volatile uint8_t sd_dma_done;
extern uint8_t sd_tx_dummy[64];
...
inline DRESULT USER_SPI_read (
    BYTE drv, BYTE *buff, DWORD sector, UINT count
)
{
    ...
    for (UINT i = 0; i < count; i++) {
        int rc = sd_read_block_dma(sector + i, buff + (i * 512));
...
#endif
/* Read one 512-byte block at sector (LBA) into buff using DMA */
static int sd_read_block_dma (DWORD sector, BYTE *buff)
{
    /* Convert to byte address if card is not block-addressing */
    DWORD addr = (CardType & CT_BLOCK) ? sector : (sector * 512UL);

    /* Select card */
    despiselect();
    if (!spiselect()) return -1;

    /* Send READ_SINGLE_BLOCK (CMD17) */
    if (send_cmd(CMD17, addr) != 0) {
        despiselect();
        return -2;
    }

    /* Receive payload via DMA */
    if (!rcvr_datablock_dma(buff, 512)) {
        despiselect();
        return -3;
    }
...
static int rcvr_datablock_dma (BYTE *buff, UINT btr)
{
    ...
    while (remaining) {
        uint16_t chunk = (remaining >= SD_DMA_CHUNK) ? SD_DMA_CHUNK : remaining;

        sd_dma_done = 0;
        if (HAL_SPI_TransmitReceive_DMA(&SD_SPI_HANDLE, sd_tx_dummy, p, chunk) != HAL_OK) {
            return 0;
        }

        SPI_Timer_On(200);
        while (!sd_dma_done && SPI_Timer_Status()) { /* spin */ }
        if (!sd_dma_done) {
